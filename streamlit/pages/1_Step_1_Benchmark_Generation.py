...
if 'uploaded_json' in st.session_state:
    ...
    input_path = os.path.join(streamlit_dir, "uploaded_files", st.session_state['uploaded_filename'])
    ...
    if 'uploaded_config_filename' in st.session_state and st.session_state['uploaded_config_filename']:
        config_path = os.path.join(streamlit_dir, "uploaded_files", st.session_state['uploaded_config_filename'])
    ...
    with col1:
        if st.button("🚀 QA 데이터 생성 시작", type="primary"):
            ...
            try:
                ...
                datamorgana_script = os.path.join(project_root, "datamorgana", "datamorgana_generator.py")
                ...
                # config 파일이 있으면 추가
                if config_path:
                    config_file_abs = os.path.join(project_root, "streamlit", config_path)
